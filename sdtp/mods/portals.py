...
class portals ( QtCore.QThread ):
    ...
    def check_for_command ( self, match_group ):
        ...
        if not match:
            self.controller.debug ( "Regex did not match." )
            matcher = re.compile ( r"^(.*): /go$" )
            match = matcher.search ( match_group [ 7 ] )
            if match:
